...
def load_inputs(period = 'historical', mem_ind = 0):
    ...
    da = xr.open_mfdataset('/rds/general/user/tk22/ephemeral/bias_corrected/'+
                           f'fAPAR_{period}_{mem}_daily.nc')['fAPAR'][0,:,:,:]
    fapar = da.to_numpy()
    ...
    return temp, vpd, co2, patm, rel_soilm, alpha, ppfd, fapar
...
def optimal_gpp_output(mem_ind, period = 'historical'):
    ...
    (temp, vpd, co2, patm, 
     rel_soilm, alpha, ppfd, fapar) = load_inputs(period = period,
                                                  mem_ind = mem_ind)
    fapar = np.nan_to_num(fapar, nan = 0, posinf = 0, neginf = 0)
    ...
    ds = xr.Dataset(data_vars = {'gpp': (['member','time','latitude','longitude'],
                                         gpp[np.newaxis,:,:,:])},
...
if __name__ == '__main__':
    try:
        wait = int(os.getenv('PBS_ARRAY_INDEX'))
        time.sleep(150*wait)
    except:
        pass
    ...
    if sys.argv[1] == 'hist':
        ...
        for i in values:
            if os.path.exists('/rds/general/user/tk22/ephemeral/bias_corrected/'+
                              f'gpp_historical_{members[i]}_daily.nc'):
                pass
            else:
    ...
    if sys.argv[1] == 'proj':
        ...
        for i in values:
            if os.path.exists('/rds/general/user/tk22/ephemeral/bias_corrected/'+
                              f'gpp_projected_{members[i]}_daily.nc'):
                pass
            else:
